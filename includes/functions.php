...
function get_term_thumbnail($attr = null)
{
    ...
    return apply_filters('get_term_thumbnail', $thumbnail);
}


function get_default_post_thumbnail_id($term_id):int|null
{
    $thumbnail_id = get_term_meta($term_id, 'default_cover', true);
    return !empty($thumbnail_id) ? $thumbnail_id : null;
}


function get_default_post_thumbnail($term_id, $attr = []):string
{
    $attach_id = get_default_post_thumbnail_id($term_id);
    if(empty($attach_id)){
        return '';
    }

    if (function_exists('kama_thumb_img')) {
        $thumbnail = kama_thumb_img(array_merge($attr, [
            'attach_id' => $attach_id,
        ]));
    } else {
